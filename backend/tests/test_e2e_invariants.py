...
async def test_e2e_financial_invariants(client: AsyncClient, db_session):
    ...
    res_a = await client.post(
        ...
        json={"email": "a_e2e@test.com", "password": "password123"},
    )
    assert res_a.status_code == 201

    res_b = await client.post(
        "/api/v1/auth/register",
        json={"email": "b_e2e@test.com", "password": "password123"},
    )
    ...
    login_a = await client.post(
        "/api/v1/auth/login",
        data={"username": "a_e2e@test.com", "password": "password123"},
    )
    ...
    login_b = await client.post(
        "/api/v1/auth/login",
        data={"username": "b_e2e@test.com", "password": "password123"},
    )
    ...
    wallet_a_res = await client.post(
        "/api/v1/wallets/", json={"currency": "USD"}, headers=headers_a
    )
    assert wallet_a_res.status_code == 201
    wallet_a_id = wallet_a_res.json()["id"]

    wallet_b_res = await client.post(
        "/api/v1/wallets/", json={"currency": "USD"}, headers=headers_b
    )
    ...
    deposit_res = await client.post(
        f"/api/v1/wallets/{wallet_a_id}/deposit",
        json={"amount": 100.0},
        headers=headers_a,
    )
    ...
    transfer_res = await client.post(
        "/api/v1/transfers/p2p", json=transfer_payload, headers=headers_a
    )
    assert transfer_res.status_code == 200

    # Verify Invariant 1: Wallet balances mathematically consistent
    wallet_a_check = await client.get(
        f"/api/v1/wallets/{wallet_a_id}", headers=headers_a
    )
    assert wallet_a_check.json()["balance"] == 7000  # 10000 - 3000

    wallet_b_check = await client.get(
        f"/api/v1/wallets/{wallet_b_id}", headers=headers_b
    )
    ...
    credit_amounts = sum(e.amount for e in entries if e.entry_type == "credit")
    ...
    res1 = await client.post(
        "/api/v1/transfers/p2p", json=idem_payload, headers=idem_headers
    )
    ...
    wallet_a_check_2 = await client.get(
        f"/api/v1/wallets/{wallet_a_id}", headers=headers_a
    )
    ...
    fail_res = await client.post(
        "/api/v1/transfers/p2p", json=fail_payload, headers=headers_a
    )
